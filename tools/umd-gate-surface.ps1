...
Write-Host ""
Write-Host "--- DDI refusals (R911 + R1010; all eleven must read 0 on a healthy session) ---"
...
foreach ($name in $sessions.Keys) {
    ...
    if ($hit) {
        ...
        # Anchored to the eleven counter names: an unanchored `(\w+)=(\d+)` also
        ...
        $names = 'srv_raw_hazard|resource_raw_hazard|text_filter_size_ignored|' +
                 ...
                 'alloc_meta_format_unknown|readback_stride_unsafe'
