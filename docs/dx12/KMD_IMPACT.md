...
And the KMD's submission DDIs forward **no** venus: `dxgkddi_submit_command` and
`dxgkddi_submit_command_virtual` (both `kmd_render/src/ddi/submit_command.rs`; ⚠ cited by symbol —
the old `:766-799` / `:725-760` now land inside `note_and_maybe_signal`, which shares the shape and
would read as confirmation) only (a) count the fence, (b) `arm_dma_flip(...)` from the DMA private
data, and (c) `note_and_maybe_signal(...)`. `grep -n "submit_venus" kmd_render/src/ddi/submit_command.rs` returns
nothing.
...
(`submit_command.rs`'s `signal_dma_completed`, `completed.SubmissionFenceId = fence`; the old `:346-350` is now the `WfBHold` counter doc). The only interrupt types
...
- **Under the vkd3d engine (both arms),** `ID3D12Resource::GetGPUVirtualAddress` returns
  `resource->res.va` (`vkd3d-proton-helios/libs/vkd3d/resource.c`'s `d3d12_resource_GetGPUVirtualAddress` / `GetVulkanResourceInfo` desc copy; the `return resource->res.va` is at `:2674`, the old `:2656-2663` names only the `resource_desc` field copy above it), which is a **Vulkan
...
| **Cross-adapter** (`D3D12_HEAP_FLAG_SHARED_CROSS_ADAPTER`) | needs `DXGK_VIDMMCAPS.CrossAdapterResource` | ❌ **OFF by default.** `cross_adapter: read_config_dword(knobs::CROSS_ADAPT_CAPS, 0) != 0` (`adapter/mod.rs:247`); `CrossAdaptCaps` is absent from the live service key. A knob flip + `pnputil /restart-device` turns it on; the cross-adapter pitch logic already exists (`create_allocation.rs`'s `pub(crate) use helios_kmd_logic::cross_adapter_pitch` re-export → `kmd_logic/src/lib.rs`'s `cross_adapter_pitch` / `CROSS_ADAPTER_PITCH_ALIGN`, with `cross_adapter_pitch_vectors` as its test) |
...
`MAX_BLOBS = 8192`, `MAX_RESOURCES = 16384`, `MAX_CONTEXTS = 1024` (all three declared together at
the top of `kmd_render/src/virtio/gpu/mod.rs`; the prose names the symbols, which is the citation —
the old `:125,129,131` had drifted to `:127,131,133`), `MAX_MAPPINGS = 8192`
(`kmd_render/src/mapping.rs`'s `MAX_MAPPINGS`).
...
- `DxgkDdiPreemptCommand` (`submit_command.rs`'s `dxgkddi_preempt_command`; was `:905-931`) drops every pending WDDM fence under the
...
- `SupportPerEngineTDR = 1`; `ABANDONED_FENCES` (`submit_command.rs`'s `ABANDONED_FENCES`, bumped in `abandon_pending_submissions`; was `:806`) counts fences discarded by
  any of the three.
...
| **Phase 0** | **13** distinct `D3DKMT*` entry points in one file, plus `D3DKMTShareObjects` in `libs/vkd3d/device.c` (the resource and fence shared-handle paths; `:7634` and `:7705` after the submodule bump, was `:7633`/`:7704`) — 14 in all, **none for rendering, submission, allocation, or hot-path fences** | `vkd3d-proton-helios/libs/vkd3d/d3dkmt.c`, 449 lines (declarations at `include/private/vkd3d_d3dkmt.h:352-366`) |
...
| `SubmitCommand` / `Render` | `DxgkDdiSubmitCommandVirtual` / `SubmitCommand` / `Render`+`Patch` | ✅ `submit_command.rs` — `dxgkddi_submit_command_virtual`, `dxgkddi_submit_command`, `dxgkddi_render`, `dxgkddi_patch` (symbols: the file grew 357 lines in the D3D12 changeset and `:725-799` no longer names any of them) |
...
|---|---|---|---|---|:--:|---|
| **K1** | Validate `NodeOrdinal` / `EngineAffinity` in `DxgkDdiCreateContext` and count refusals (`CtxNode`) | A context for a node that does not exist is accepted **silently** today. `DxgkDdiCreateHwContext` already checks (`scheduler.rs:135-137`). AGENTS.md rule 2: every skipped/refused path gets a named counter | `device.rs:389-431` (no check) vs `scheduler.rs:135-137` (checked) | **S** | No | **Low** — but it is a *new refusal on a live path*. ⚠ Ship the counter first, the refusal second, once evidence shows today's callers always pass 0 |
| **K2** | Set `ContextInfo.Caps.NoPatchingRequired = 1` and shrink `AllocationListSize`/`PatchLocationListSize` for contexts created with `DXGK_CREATECONTEXTFLAGS.VirtualAddressing` | The documented shape for a GPU-VA context: *"no patch location list … and only a very small allocation list (16 entries)"*. Helios asks 256+256 on **every** context and no-ops `DxgkDdiPatch` | `device.rs:414-428`; `d3dkmddi.h:1550-1580`; `submit_command.rs`'s `dxgkddi_patch` (the no-op; was `:1421-1430`, which is now `dxgkddi_render`'s D3D12 merge-failure arm) | **S** | No | **Medium** — changes what dxgkrnl allocates per context for *every* client including DWM, and the allocation list is how the Present path receives its surfaces. ⚠ Do it last, behind a knob, with a paired GT1/GT2 measurement |
...
the used-ring drain) before and after a D3D12 run. Do this first; it costs one run.~~
...
4. ⛔ **A ring-0 wire fence is not even a venus decode fence on this host.** The KMD sets
   `VIRTIO_GPU_FLAG_INFO_RING_IDX` only for `ring_idx != 0` (`gpu/mod.rs`'s `enqueue_submit_inner`,
   the one site that stamps the SUBMIT_3D header; was `:3482`), and QEMU routes a
...
context, that **`D12Rec`** moves (§14a.4 item 3's free settlement of **P7**), that the three
windows come back, and that nothing bugchecks. ⛔⛔ **NOT `RENDER_COUNT`, and this sentence used to
say `RENDER_COUNT` ~30 lines above the place §14a.4 item 3 strikes it.** `RENDER_COUNT` is
adapter-global, bumped from three sites in `submit_command.rs`, and DWM's D3D11 present path calls
`pfnRenderCb` every frame — so it moves continuously with no D3D12 client at all. It also has **no
reader on a default deployment**: its only two readers are `diag_dump_engine_atomics`
(`submit_command.rs`, which 16-bit-truncates it — `RENDER_COUNT … & 0xFFFF` — and runs from
`DxgkDdiDestroyDevice`) and index 6 of the `'HDBG'` `DxgkDdiCollectDbgInfo` report, i.e. provoke a
TDR and decode a minidump. `D3D12_SUBMIT_RECORDS` / **`D12Rec`** is the instrument this changeset
built for exactly this question: it is bumped only on the arm that decoded a valid
`HeliosD3D12SubmitCmd`, so it is D3D12-scoped by the `'HE12'` magic rather than by hope.
**UV1's only clean test is a deliberate KMD-side
...
(`gpu/mod.rs`'s `SCANOUT_RING_IDX`), so `RING_SUBMIT_COUNT` is dominated by **this driver's own**
scanout and BLT copies — three internal producers, not two, and they are named by **symbol**
because the line numbers this row used to carry (`:3375`, `:3442`, `ctrl.rs:1541-1547`) all drifted
in the 983-line growth: `gpu/mod.rs`'s `enqueue_async_submit_windowed_blt` and
`enqueue_scanout_submit` (both hard-code `SCANOUT_RING_IDX`), plus `ctrl.rs`'s
`submit_venus_async_present`, which passes `SCANOUT_RING_IDX` to the generic `enqueue_async_submit`.
⚠ `gpu/mod.rs`'s `enqueue_async_submit_present_stream` is a **fourth** submit site but is *not* a
fourth internal producer — its ring is the guest's, forwarded. Nor were those counters readable at
all: they appeared only in the
...
| **K-F1** | The submission: drain (`vkd3d_acquire_vk_queue`) → `pfnRenderCb` on `QueueState::h_context` → re-latch. ⛔ **"No boundary record" is SUPERSEDED** — it carries `HeliosD3D12SubmitCmd` from the start, with `gpu_wire_fence = 0` until ICD-1 lands, because the record's presence is what scopes K-F0 and a second payload shape would be one more thing to keep in sync. ⭐ **Zero KMD change.** ⛔ **But it does NOT make `ID3D12Fence` truthful on its own, and it is not the UV1 test** — with a zero fence it takes the final fall-through in `gpu/mod.rs`'s `note_wddm_submission` (`(self.next_wire_fence, WireBoundary::Prefix)`; was cited `:5641`), which §14a.1's correction shows is **already satisfied** during a D3D12 frame. What it settles is the **plumbing**: dxgkrnl accepts `pfnRenderCb` on a legacy D3D12 context, **`D12Rec`** moves (**P7**, free — ⛔ **not `RENDER_COUNT`**, which is adapter-global, DWM-dominated and unreadable on a default deployment; see §14a.1's correction and §14a.4 item 3), the windows come back, nothing bugchecks. | `umd12` | S | **[C]** |
| **K-F0** | ⛔ **NO LONGER CONDITIONAL** (was *"only if K-F1's reading is flat"*): per §14a.1's correction, K-F1's unheld reading cannot answer **UV1** at all, so the hold is the *only* clean test and lands in the same KMD deploy. `WddmHoldMs` + a fourth block arm in `gpu/mod.rs`'s `take_one_ready_wddm` (three arms already, exactly one counter per blocked look; the old `:5735-5752` is now `cancel_windowed_blt_for_resource`) + `WfBHold`, released by the existing 60 Hz DISPATCH heartbeat (`adapter/kobj.rs`'s `service_vsync_tick`, which now reads `WDDM_HOLD_MS`/`WDDM_HEAD_MS`) with one added `request_wddm_completion_dpc`. ✅ **This arm is IN THE TREE as of this changeset** — `take_one_ready_wddm`'s fourth, deliberately-last `if hold_until != 0` arm, `diag.rs`'s `WDDM_HOLD_MS` knob (default 0 = OFF) and `virtio/counters.rs`'s `WfBHold`. What remains unlanded is the *reading*, not the code. ⛔ **Scoped by the presence of `HeliosD3D12SubmitCmd`** — carried as a flag on `WddmPending` — or the adapter-global head-of-line FIFO stalls the whole desktop, DWM included. N ≪ TdrDelay, **clamped in code**: an operator typo must not be a TDR. **Reading:** N grows by ~the hold ⇒ **UV1 ✓**; N stays at the 0.8–1.1 µs baseline ⇒ **UV1 ✗**, and none of this design is the answer. | `kmd_render` | S | **[E]** |
| **K-F2** | ⛔ **A LIVE DEFECT ON THE SHIPPING PRESENT PATH, independent of D3D12 — but ⛔⛔ NOT the fix this row prescribed until 2026-08-06.** `gpu/mod.rs`'s `present_stream_marker_boundary` validates `ctx_id`/`value != 0`/`cookie`/`creator_process` and slot liveness, and bounds `value` in **no** way, so a guest writing an absurd `present_value` gets a *live* boundary `present_stream_slot_ready` can never satisfy; `wddm_pending` is an **adapter-global head-of-line FIFO** (`take_one_ready_wddm`, never bypassed), so it blocks every context including DWM's — bounded only by the FIFO's own 256-entry overflow escape (the `wddm_pending.len() >= MAX_WDDM_PENDING` arm of `note_wddm_submission` → `overflow_wddm_pending`, `WDDM_PENDING_OVERFLOWS`, which then *drops* fences) or TDR, whichever lands first. ⚠ **Every line number this cell used to carry has drifted and they are symbols now** (was `:4721-4744`, `:945-953`, `take_one_ready_wddm:5718-5765`, `:5664-5685`, `prepare:4770`, `commit:4790`). ⛔⛔ **Copying the tag-path comparison (`value <= slot.submitted_value`) is REFUTED**: the tag path (`prepare_present_stream_tag` / `commit_present_stream_tag`) is the PRODUCER advancing the stream and requires the new value to be strictly AHEAD; the marker is a CONSUMER predicate, and on the shipping default the marker is delivered **before** the frame's `vkQueueSubmit` **by design**. `UmdAsyncPresentStream` is absent = ON (`umd/src/knobs.rs`'s `UMD_ASYNC_PRESENT_STREAM`), and `async_stream_eligible` **skips** `HeliosWaitFrameSubmitted` (`umd/src/forward/present.rs:1479-1528`) *because* the marker is what carries the dependency; the value is minted on the app thread (`umd/bridge/dxvk_bridge.cpp:1316`) while the tag that sets `submitted_value` rides DXVK's submission thread (`HeliosSignalPresentFence` is `EmitCs` only — `d3d11_context_imm.cpp:1150-1162` → `vn_queue.c:1994` → `:1736-1744`). So `value == submitted_value + 1` is the steady state, the check would refuse ~every legitimate frame, and each refusal falls back to `wire_fence_watermark()` (`adapter/scanout.rs:256-270`) — which does **not** cover the unsubmitted frame. With the UMD's gate skipped too, that is the **0ab-B stale/black-frame class returning**, plus `PresentWmk` silently demoted: a correctness regression, not a hardening. ⇒ **the guard is consumer-side liveness** — bound how long the FIFO head may block on a stream boundary, then rebase to the legacy watermark, loudly counted (K-F0's fourth-arm plumbing in `take_one_ready_wddm`), which also covers "submitted but never retired". An acceptance-side lookahead bound cannot stand alone: legitimate lookahead reaches DXVK's `MaxNumQueuedCommandBuffers = 32` (`dxvk-helios/src/dxvk/dxvk_limits.h:17`), and a forged value whose process then stops presenting is unsatisfiable at any bound. Its own commit; its own counter (**not** `PRESENT_STREAM_REJECTS`, which every other refusal already shares). ✅ **The prescribed repair IS in the tree as of this changeset** (it landed as A5's half): `diag.rs`'s `WddmHeadMs` knob (default **250 ms** = ON, `0` the A/B disable) bounds how long the FIFO head may block, `gpu/mod.rs`'s `rebase_blocked_head` rebases to the conservative wire watermark, and `WfBReb` counts it — with `present_stream_marker_boundary`'s two magnitude instruments added alongside, deliberately as INSTRUMENT ONLY (nothing there refuses; the boundary it returns is byte-identical to before). ⛔ The wire arm is deliberately **not** rebasable — read `take_one_ready_wddm`'s `WDDM_HEAD_BLOCKED_WIRE` comment before proposing it. | `kmd_render` | S | **[C]** |
| **K-F5** | Counters for the new arm, atomics only (DISPATCH), mirrored from the PASSIVE site `record_present_handoff_telemetry` (`submit_command.rs`; was `:108-181`, now `:179`ff). The `PmHit`/`PwExact`/`WfB*` family is the template. ⛔⛔ **Read `PENDING.md`'s STATUS 2026-08-07 block before reading any of these counters**: that function is their SOLE writer, publishes at `DiagLevel = 0` once per **600 queued refreshes**, and is unbounded while DWM is idle — so a short probe reads a stale zero, not a finding. | `kmd_render` | XS | **[C]** |
| **ICD-1** | ⛔⛔ **REPLACED 2026-08-06, not relocated.** ~~`helios_venus_last_submitted_wire_fence(VkDevice, …)` reading a new `last_wire_fence_id` stored at `vn_renderer_helios.c:1739`~~ would return **whatever `SUBMIT_VENUS` happened last, process-wide, on any thread** — and for a vkd3d workload every submission that reaches that line is `ring_idx == 0` (§14a.1's correction): the ring doorbell, `vn_ring_submit_roundtrip`, ring create/destroy, the wait-alloc ordering batch. The KMD would then take it as a `gpu_completion_fence` and gate on `watermark = id + 1` (⚠ **that decision table has MOVED OUT of `gpu/mod.rs` since this row was written** — it is `helios_kmd_logic::wddm_boundary::select` in `kmd_logic/src/lib.rs`, precisely so A4 and A6 could be tested; `gpu/mod.rs`'s `note_wddm_submission` only maps its verdict onto counters and onto `WireBoundary`. The old `:5606-5610` is now inside the windowed-BLT cancellation path) — **a fence that resolves at decode at best, and on this host at an empty `glFenceSync` in QEMU's GL context 0.** It would make the D3D12 fence *look* ordered while ordering nothing, and nondeterministically, since it names another thread's submit. ⇒ The shape is **per-queue**: `helios_venus_queue_gpu_fence(VkQueue, uint64_t *out_wire_fence)`, which encodes `vkWaitRingSeqnoMESA(vn_ring_get_id(dev->primary_ring), vn_ring_current_seqno(...))` — the barrier proving the host issued our `vkQueueSubmit` — and submits it with `.ring_idx = queue->ring_idx` (`vn_queue.h:26`, assigned `vn_device.c:83-88`, **never 0**), returning the KMD-assigned wire fence. Template: `vn_signal_win32_external_semaphore` (`vn_queue.c:1710-1755`). ⚠ A non-empty cs is **mandatory** — `helios_submit` skips the escape entirely when `cs_size == 0` (its `if (batch->cs_size)` gate) and no fence id comes back; and it discards the id unless a sync is attached (the `batch->sync_count` loop right after). ⛔ Must refuse `ring_idx == 0` loudly: the host fails a fence on an unbound/out-of-range ring (`vkr_context.c:151-155`, virglrenderer, not checked out here), the virtio response never comes, and the KMD's in-flight entry is **immortal** — the wedge documented on `gpu/mod.rs`'s `latch_failed_and_fail_inflight`. ✅ **LANDED** (ICD `49acb236`, refined by `6993dd4a`): `helios_venus_queue_gpu_fence` is exported and the `HELIOS_QUEUE_GPU_FENCE_RING_LIMIT = 64` bound + the unconditional ring-0 refusal are both real code, not asserts. ⛔⛔ **TWO of this row's prescriptions were REVERSED by the implementation, and the reasons are in the source, not here.** (a) It does **NOT** call `helios_ioctl_submit_cs` and does **NOT** take `dev_mutex` — that is defect **A3**, and `helios_submit_gpu_fence_cs` stages the whole SUBMIT_VENUS escape **on the stack** so it needs no lock; its header enumerates, item by item, what `dev_mutex` actually protects on the submit path and why none of it applies. (b) The per-ring fence cache this design implied was **deleted** — its key could never match (defect A2); only the ring *bound* was load-bearing. ⚠ All five ICD line numbers this row used to carry (`:1739`, `:3038-3044`, `:3037-3060`, `:454`) are stale — the submodule was bumped in this changeset — and are symbols now. | `icd/mesa` | S | ✅ |
| **K-F3** | ✅ **LANDED 2026-08-06** (`629d1da`) as `HeliosD3D12SubmitCmd` — `{magic `'HE12'`, version, gpu_wire_fence: u64}`, **16 bytes**, declared once in `protocol/src/wddm.rs` per D13. ⭐ The length is load-bearing in **both** directions — it must reach 16 to be recognisable at all (`DxgkDdiRender` gates its refresh decode on `cmd_len >= 16`) and stay under the 48-byte typed-scanout prefix so that arm is never attempted. ⛔⛔ **BUT "the `const _` block asserts both" IS FALSE, and it was false when written.** Read `protocol/src/wddm.rs`'s `const _` block: `HeliosD3D12SubmitCmd` gets one **equality** (`size_of == 16`) and exactly one **relational** assert, and that relation is the **upward** one — `size_of::<HeliosD3D12SubmitCmd>() >= offset_of!(HeliosPresentRefreshCmd, present_ctx_id)`, i.e. ≥ 16. **Nothing relates 16 to the 48-byte typed-scanout prefix**, so the downward direction is asserted by no one; the comment beside the assert even names the wrong symbol for it (`size_of::<HeliosPresentRenderCmd>()` is **80**, not 48 — 48 is the decode PREFIX `DxgkDdiRender` uses). ⇒ **the real guard is the magic word, which the rest of this row and both magic constants' own doc comments already say correctly.** Both existing arms reject on magic; length only decides which decodes are *attempted*. ⛔ Not `HeliosPresentRenderCmd` — `HeliosPresentPrivateData::is_valid()` requires `resource_id != 0` and an ECL has no primary. ⛔ Not `HeliosPresentRefreshCmd` — its arm arms a scanout refresh for every valid `'HERF'` record (`submit_command.rs`'s `arm_scanout_refresh_after_current_venus(adapter, 0, stream_marker, false)` inside `dxgkddi_render`'s HERF block; ⚠ the old `:1064` now lands in the **TDR/preempt abandon** path, which would tell a re-deriving reader the opposite of what this row claims), which a compute or graphics ECL must not do. ⭐ `is_valid()` deliberately does **not** check the fence: `0` is legal and means *"submit the packet, order it against nothing"* — the plumbing arm and the A/B disable — and the record's **presence**, not its content, is what scopes K-F0's hold to this path instead of the adapter-global FIFO. ⭐ A guest-supplied fence is safe by asymmetry: the clamp replaces a value at or beyond `next_wire_fence` with `next_wire_fence` itself, so the worst case is naming an *earlier* fence and under-waiting (a lie, with a counter) — no unsatisfiable boundary, and therefore no wedge, is reachable from this field. ⚠ **The clamp is NOT at `gpu/mod.rs:5609-5616` and no longer lives in `gpu/mod.rs` at all**: it is `helios_kmd_logic::wddm_boundary::select`'s `Rejection::OutOfRange` arm (`kmd_logic/src/lib.rs`), moved there because A4 and A6 are both defects *of that table* and `kmd_render` is a `panic="abort"` cdylib that cannot host a test — `wddm_boundary_tests` is the oracle. `gpu/mod.rs`'s `note_wddm_submission` only maps the verdict onto `GPU_FENCE_CLAMPED`/`GPU_FENCE_FOREIGN_GENERATION` and `WireBoundary`. The old range is now `cancel_dead_undispatched_windowed_blt`. | `protocol` | S | ✅ |
| **K-F4** | Decode K-F3 in `dxgkddi_render` and write the wire fence into `PresentSubmissionPrivate.gpu_fence_id` via `merge_fence` (`present_packet.rs`'s `merge_fence`; was `:299-344`). ⚠ **`dxgkddi_render` never touched `pDmaBufferPrivateData`** — every hit in `submit_command.rs` was on the `submit` arg. `DXGKARG_RENDER` has both fields. Write at offset 0, do not advance the pointer, which is the shape Present already proves. ⭐ Everything downstream is already correct **and already guarded**: `helios_kmd_logic::wddm_boundary::select` rejects a guest value `>= next_wire_fence` — *"must not manufacture an impossible future dependency"* — and counts it (`GpuFncClamp`). ✅ **LANDED as of this changeset**: `present_packet.rs` now carries `mark_d3d12` **and** `invalidate_d3d12`, and `dxgkddi_render` writes offset 0 on the ECL arm. ⛔ The pairing matters: a Render that does **not** write an `'HD12'` record must CLEAR one, or UP-9's second `pfnRenderCb` on the same context lets a stale record be inherited (`D12Clr` counts the clears). ⚠ **There are TWO D3D12 magics and they are not interchangeable** — `'HE12'` (`HELIOS_D3D12_SUBMIT_MAGIC`, `protocol/src/wddm.rs`) is what the UMD puts in the *command buffer*; `'HD12'` (`D3D12_SUBMISSION_MAGIC`, `present_packet.rs`) is what the KMD stamps into *DMA private data*. `D12Rec` counts the former's `is_valid()`; `D12Clr` counts clearing the latter. | `kmd_render` | S | ✅ |
| **K-F6** | ⭐ **THE GPU TIMESTAMP CLOCK — added 2026-08-07, and it should have been on this list from the day `SPECS.md` logged it.** ⛔⛔ **This document had ZERO occurrences of "timestamp" or "CalibrateGpuClock" while the clock was the largest KMD item the changeset shipped**, and `PENDING.md` §6 already records that this exact item *"fell through"* once before — logged against `KMD_IMPACT.md`, which then did not carry it. Two identical misses is a process finding, not an oversight. **The defect:** `dxgkddi_calibrate_gpu_clock` zero-filled `DXGKARG_CALIBRATEGPUCLOCK` and returned `STATUS_SUCCESS`, so `GpuFrequency = 0` and **a benchmark scores zero regardless of how correct everything else is** — a rule-2 violation (fabricates data, returns success, no counter) on the exact path a score depends on. **The answer, and why it is self-consistent rather than merely plausible:** `GPU_TIMESTAMP_FREQUENCY_HZ = 1_000_000_000` in `kmd_render/src/ddi/scheduler.rs`, because vkd3d answers its own `d3d12_command_queue_GetTimestampFrequency` as `1e9 / timestampPeriod` and this guest reports `timestampPeriod = 1` — i.e. the value the engine underneath us would have reported for the same queue. ⛔ **`GpuClockCounter` STAYS 0, deliberately, and must not be "fixed".** Synthesising it from an interrupt-time source is the right *rate* with the wrong *epoch*: every GPU↔CPU correlation goes silently wrong while `ClkCal`/`ClkNoGpu`/`ClkFreq` all read healthy. `CpuClockCounter` **is** answered honestly (QPC). ⛔⛔ **And a diag record here is FORBIDDEN, not merely omitted** — `DXGKDDI_CALIBRATEGPUCLOCK` is `_IRQL_requires_max_(DISPATCH_LEVEL)` and *"called on timer"*, so the registry write AGENTS.md's rule 2 would otherwise ask for is illegal at this IRQL. The counters are therefore atomics (`GPU_CLOCK_CALIBRATE_CALLS`, `GPU_CLOCK_NO_GPU_COUNTER`) mirrored at PASSIVE via `gpu_clock_counters()` → `record_present_handoff_telemetry`, surfacing as **`ClkCal`** (graded EXPECTED-NONZERO — a zero across a D3D12 run means the DDI is never entered and S-1 is open elsewhere), **`ClkNoGpu`** (graded EXPECTED-EQUAL-TO-`ClkCal`; a gap means somebody landed a real GPU-clock source and did not re-grade) and **`ClkFreq`** (the answer itself, in Hz, so the assumption is readable). ⭐ These three are **boot-cumulative and are NOT hidden by the 600-refresh publish throttle** that hides the other new counters. ⚠ **What would make the answer wrong:** a host GPU whose `timestampPeriod != 1` — it is a property of the physical device, not of Venus (AMD 40 ns, Intel 83.333 ns), and then every timestamp delta an application computes is off by exactly that factor, silently. The honest fix is the escape carrying the ICD's own `timestampPeriod` into the KMD (`PENDING.md` §2 S-1 sizes it **M**). ⭐ **Its sibling was fixed in the same pass, and the reason is a lesson about superlatives.** The original comment called this *"the ONE DDI in this driver that fabricates data, returns success and has no counter"* — **false when written**: `dxgkddi_set_stable_power_state` (same file) returns `void`, so dxgkrnl reports success no matter what, and it did nothing and counted nothing, while `SetStablePowerState(TRUE)` is the first call of every D3D12 timing harness. A superlative in a comment is a claim about the rest of the file, and this one would have retired a live defect by asserting it did not exist. ⚠ **`PENDING.md`'s STATUS-2026-08-07 block still lists it as *"an empty body returning void … with no counter"* — that is now STALE**: it is counted, as **`StblPwr`** / **`StblPwrEn`**. The no-op itself is probably correct (there are no guest clocks to lock and virtio-gpu carries no clock-control command); the defect was that a no-op was indistinguishable from a working one. ⭐ **DWM never asks for a stable power state, so unlike almost every counter in this driver these two are attributable to the profiling client with no control arm.** Grading: `StblPwr == 0` over a benchmark run means nobody asked and nothing is fabricated; **`StblPwrEn > 0` is the loud reading** — that many callers were told their GPU clock is pinned and it is not, so any conclusion drawn from clock stability in that run is void. | `kmd_render` | S | ✅ |
...
full**, with compile-time asserts (`present_packet.rs`'s `const _` block, beside
`PRESENT_DMA_PRIVATE_DATA_BYTES`; was `:231-241`) written to force the issue. Reusing
`gpu_fence_id` costs zero bytes.

⚠ **One silent-lie hazard to guard**: `DmaGpuFence=0` routes a D3D12 packet to
`RetireDomain::DecodeOnly` (the `paging || !self.dma_gpu_fence` arm of `gpu/mod.rs`'s
`note_wddm_submission`; was `:5599`) and makes the fence lie quietly. Default is 1.
...
| **UP-2** | ⭐ **A ~20-line vkd3d method, not an M-sized patch.** `ID3D12DXVKInteropDevice3::GetVulkanHeapInfo` (`libs/vkd3d/device_vkd3d_ext.c:1123-1146`) already returns `{VkDeviceMemory, offset, vk_memory_type}` for an `ID3D12Heap`; a **committed** resource has no `ID3D12Heap`, and `GetVulkanResourceInfo1` returns the `VkImage`/`VkBuffer`, not the memory. Add the sibling — an `ID3D12DXVKInteropDevice4::GetVulkanResourceMemoryInfo(ID3D12Resource*, …)` reading `resource->mem.device_allocation.{vk_memory, vk_memory_type}` and `.offset` — same file, same shape, same guards. ✅ **LANDED 2026-08-06**, submodule `4c26d855`. The ~20-line estimate held for the body (23 non-comment lines) and missed the change: 72 inserted over 4 files, of which 27 are the IDL block, because a new COM revision moves five sites together (IDL + uuid, `vkd3d_private.h`'s `typedef ID3D12DXVKInteropDevice4 d3d12_dxvk_interop_device_iface` (was cited `:5466`; `vkd3d_private.h` gained 35 lines in the bump and `:5466` is now an unrelated struct field), the vtbl type and entry, `device.c`'s `extern CONST_VTBL struct ID3D12DXVKInteropDevice4Vtbl d3d12_dxvk_interop_device_vtbl` (was `:11403`), `device.c`'s `IID_ID3D12DXVKInteropDevice4` QueryInterface arm). No `.def` change — COM interfaces are not DLL exports. Two corrections to this row's own prescription: ⭐ it also returns **`memory_size`**, which is the `VkMemoryAllocateInfo::allocationSize` (`memory.c`'s `allocate_info.allocationSize = size`) and therefore exactly `HeliosWddmAllocMeta::venus_alloc_size` — a caller cannot derive it, because the allocator suballocates, and returning it removes one of the two reasons UP-5 needs the ICD at all. ⛔ And **`resource->mem` is the wrong field for a whole resource class**: a CPU-accessible texture takes `VKD3D_RESOURCE_LINEAR_STAGING_COPY` (`resource.c`'s single site setting `VKD3D_RESOURCE_LINEAR_STAGING_COPY` on `object->flags`; was `:4229`, now `:4241`) and binds its **image to `private_mem`** while `mem` holds the host-visible linear staging buffer (the two `object->flags & VKD3D_RESOURCE_LINEAR_STAGING_COPY` bind sites further down the same file; was `:4491-4503`, `:4729-4738`), so the obvious three-line body this row describes would return staging memory with an `S_OK`. The method branches with the bind site instead; the contract is *"the memory the image or buffer is BOUND to"*. | vkd3d fork | S | ✅ |
| **UP-3** | Force **dedicated + venus-exportable** memory for resources created under `D3D12DDI_HEAP_FLAG_PRIMARY`. Two hazards, both real: vkd3d suballocates committed textures unless `prefersDedicatedAllocation` (`libs/vkd3d/resource.c`'s `vkd3d_allocate_resource_memory` heap-flag chain; was cited `:4434-4461`) — one venus resid covering several D3D12 resources breaks both the one-resource-one-allocation rule and D3D11's `memory_offset == 0` precondition (`umd/src/forward/resource.rs:488-490`); and vkd3d chains `VkExportMemoryAllocateInfo` only for `D3D12_HEAP_FLAG_SHARED`, which a back buffer does not set. ⛔ Do **not** reach it by passing `SHARED` through — `VK_KHR_external_memory_win32` is absent on this device and vkd3d chains the export info unguarded. ✅ **LANDED in the fork** (`9410a5b3`, the submodule bump in this changeset's final commits): `VKD3D_HEAP_FLAG_HELIOS_VENUS_EXPORT` (`vkd3d_private.h`, bit 30) selects a **third** arm beside SHARED and the default — it sets `prefersDedicatedAllocation = VK_TRUE` and chains `VkExportMemoryAllocateInfo` with **`OPAQUE_FD`**, not `OPAQUE_WIN32`, so it structurally cannot reach the NULL `vkGetMemoryWin32HandleKHR` PFN this row warns about (the call-site guard tests both `heap_flags & SHARED` **and** `handleTypes == OPAQUE_WIN32`); the matching `VkExternalMemoryImageCreateInfo` is declared on the image in the same file (VUID-VkMemoryAllocateInfo-pNext-00639). `umd12` reaches it through `resource12.rs`'s `PrimaryTranslation::VenusExport` (see UP-4). ⛔ **It has still never EXECUTED** — read and coherent, unrun. ⛔⛔ **This bit is the fork's highest-risk divergence from upstream**: it is hand-mirrored between `vkd3d_private.h`'s `VKD3D_HEAP_FLAG_HELIOS_VENUS_EXPORT` and `resource12.rs`'s `HELIOS_HEAP_FLAG_VENUS_EXPORT`, and **nothing checks the two agree**. Both read `1 << 30` today (re-derived 2026-08-07); a divergence would be silent, and the symptom would be an unadopted back buffer, not an error. | vkd3d fork | M | ✅ |
| **UP-4** | The resource→identity table: every `pfnCreateHeapAndResource` records `{engine res*, vk_memory, offset, size, venus res_id, venus_alloc_size, memory_type_index, geometry, is_primary}`. Stays local to `umd12` per D13's refinement. ✅ **LANDED 2026-08-06** as `umd12/src/forward12/identity12.rs` — bounded 64 entries, keyed on `ID3D12Resource::as_raw() as usize` and **never dereferenced**, retired in `destroy_heap_and_resource` by reading the address out of the state box *before* the drop (after it, the address may already be recycled); `IdentityRecorded − IdentityRemoved` is the live count, so a leak needs no extra instrument. ⛔⛔ **BOTH of this row's "already detected" claims were wrong, and in opposite directions.** (a) `D3D12DDI_RESOURCE_OPTIMIZATION_FLAG_PRIMARY = 4` is **not usable at the create**: that type appears on exactly one DDI slot family, `pfnCheckResourceAllocationInfo` (all six generations, `d3d12umddi.rs:51734`, `:59866`, `:75022`, `:76696`, `:79414`, `:87548`) — a *sizing query with a description and no handle*, about a resource that does not exist yet. `D3D12DDIARG_CREATERESOURCE_0109` has 16 fields and none of that type (`:87456-87473`). "Use both" is not implementable where this row asks for it. (b) The signal this row calls settled is **UNMEASURED**: `HeapPrimaryFlagDropped` reads **0 in all 150** logged `umd12` runs under `tmp/`, while `ResourceOptimizationFlagsIgnored` reads **1..3 in 101 of the 150**. ⇒ it is entirely possible that the channel this row dismisses is the only one this runtime uses and the one it trusts never fires. Both are now split into per-signal counters (`ResourceOptimizationPrimary` at the slot that actually carries it) so the next run decides it; the description-matching correlation is deliberately **not** written on speculation. ⛔⛔ **A CITATION-DRIFT CORRECTION THAT ITSELF DRIFTED, and then the CLAIM under it expired too.** *"the PRIMARY drop is at `resource12.rs:659-674`, not `:630-645`"* — **both numbers are now wrong**, and worse, **there is no longer a single "PRIMARY drop"**: `D3D12DDI_HEAP_FLAG_PRIMARY` is handled in `resource12.rs`'s `heap_flags(flags, primary: PrimaryTranslation)` helper, which branches. On the **committed** arm (`create_heap_and_resource` → `CreateCommittedResource3`, `PrimaryTranslation::VenusExport`) the flag is **no longer dropped at all** — UP-5 turned it into `HELIOS_HEAP_FLAG_VENUS_EXPORT`, the fork's private request for exportable dedicated memory, counted `HeapPrimaryVenusExport`. `PrimaryTranslation::Dropped`, which is what still bumps **`HeapPrimaryFlagDropped`**, is reachable **only from the heap-only arm** (`create_heap`), where the declaration cannot be honoured because there is no resource to hand a kernel allocation to. ⇒ **`HeapPrimaryFlagDropped` is no longer the census of primaries and must not be read as one**; its partner is `HeapPrimaryVenusExport`, and `ResourceOptimizationPrimary` is the tie-breaker. This row is cited by **symbol** from here on — `heap_flags` and the two `PrimaryTranslation` variants — because two successive line-number repairs of the same three lines is the pattern `caps12.rs`'s module doc exists to stop. | `umd12` | M | ✅ |
| **UP-5** | Call the corelayer `pfnAllocateCb` (`D3D12DDICB_ALLOCATE_0022`) for presentable resources with `kind = DEVICE_MEMORY`, `adopt_resource_id`, `Flags = PRIMARY`, `Reserved[5]` **zeroed** (`Reserved fields in D3D12DDI_ALLOCATION_INFO_0022 were not zero.` is a runtime string), then `helios_venus_memory_transfer_resource_ownership` (`vn_renderer_helios.c:806`). Unwind on failure. ⛔⛔ **THE RISK THIS ROW DOES NOT NAME, and it is a silent wrong result rather than an error:** `D3D12DDICB_ALLOCATE_0022` carries `hResource: HANDLE` at offset 16 (`d3d12umddi.rs:60044-60051`) — the same association D3D11 fills with `alloc.hResource = h_rt.handle`, where `umd/src/forward/resource.rs:364-370` states that *"the association itself is **not optional** for present-only allocations"*, which is exactly what a presentable back buffer is. But `create_heap_and_resource` takes the handle as **`_h_rt_resource` and discards it**, and `ResourceState`'s doc records as a *rule* that `D3D12DDI_HRTRESOURCE` is never stored because *"Nothing in the corelayer or kernel callback tables takes an `HRTRESOURCE`"*. That premise is false in the one callback UP-5 is built on. ⇒ UP-5 must capture the handle at the create, add the field, and **revise that rule** — three edits in a file whose comments currently argue against all three. Without it dxgkrnl gets an allocation tied to no resource and the KMD's open path finds nothing to stamp identity onto. ⛔ **And "unwind on failure" omits the destroy path entirely**: once `pfnAllocateCb` succeeds this driver owns a WDDM allocation, so `pfnDestroyHeapAndResource` must call `pfnDeallocateCb` (present at `_0062` index 13, beside `pfnAllocateCb` at index 12 — so no `device12.rs` change is needed to reach either) or every back buffer leaks one allocation per swapchain resize, which is the 54th session's leak class reintroduced. | `umd12` | M | 1 |
...
3. **`P7` — does `DxgkDdiRender` fire on the D3D12 path — is settled by the fence work for free**,
   ...
   the whole test.~~ `RENDER_COUNT` is **adapter-global and incremented from three sites** in
   `submit_command.rs` (⚠ cited as `:1046`, `:1377`, `:1434` after drifting from `:996`; they have
   since drifted again, to `:1233`, `:1674`, `:1731` — **that is three stale derivations of the same
   three sites in one document, which is the argument for symbols**), and DWM's own D3D11 present
   path calls `pfnRenderCb` on every frame (`umd/src/forward/present.rs`'s present `pfnRenderCb`
   call). So it moves continuously with no D3D12 client at all. ⛔ **And it has no reader on a
   default deployment**: `diag_dump_engine_atomics` truncates it to 16 bits (`… & 0xFFFF`) and runs
   from `DxgkDdiDestroyDevice`, and the only other reader is index 6 of the `'HDBG'`
   `DxgkDdiCollectDbgInfo` report — i.e. provoke a TDR and decode a minidump. P7 is settled by the
   **record-seen counter** on K-F3's decode arm, which shipped as `D3D12_SUBMIT_RECORDS` /
   **`D12Rec`**, bumped only inside `HeliosD3D12SubmitCmd::is_valid()` and therefore D3D12-specific
   by the `'HE12'` magic rather than by construction-in-principle. **Read `D12Rec`, never
   `RENDER_COUNT`** — and see the correction under §14a.1, which this document had to make twice.
...
2. **§1.4: the `d3d12` grep hit is at `create_allocation.rs:1436-1438`.** It is at
   `create_allocation.rs`'s `cross_adapter_pitch` re-export block, whose logic now lives in
   `kmd_logic/src/lib.rs` as `cross_adapter_pitch` / `CROSS_ADAPTER_PITCH_ALIGN`. ⚠ Cited by symbol:
   this pointer has been re-derived twice (`:1436-1438` → `:1438` → drifted again), and
   `kmd_logic/src/lib.rs` grew 631 lines in the D3D12 changeset, which moved the sibling too. ⭐ The
   `d3d12` *identifier* in `kmd_logic` today is something else entirely — `wddm_boundary::select`'s
   `d3d12: bool` parameter — so a bare `grep d3d12` no longer finds what this row is about.
