...
| ~~7.9~~ | ✅ **SETTLED 2026-08-05 — `widl` is already on the VM**, from the WinLibs UCRT mingw64 distribution installed via WinGet: `…\WinGet\Packages\BrechtSanders.WinLibs.POSIX.UCRT_…\mingw64\bin\widl.exe`, and it is on `PATH`. `glslangValidator` likewise, from `C:\VulkanSDK\1.4.350.0\Bin`. The vkd3d clang-cl configure found both with no extra install (§7.29). ⚠ The CI half of the question — which package to install on a GitHub `windows-2022` runner — is still open, but it is now a CI-packaging question, not a blocker for local builds. | Closed for the dev VM; re-ask at the CI change (§6.2). |
