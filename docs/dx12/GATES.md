...
`docs/dx12/DDI_REFERENCE.md` §15. ⚠ `tmp/` is gitignored, so — as with G2's known-fail lists — the
*analysis* is mirrored into the tree at **`docs/dx12/baselines/d12-g5-contract.md`**; only the raw
multi-megabyte logs stay under `tmp/dx12/gates/G5/`.

#### ✅ PASSED 2026-08-05 — `tmp/dx12/gates/G5/answers.md`

Instrument: **`tools/d3d12_spy/`**, not a single `tools/d3d12_warp_spy.cpp` — a generator
(`gen_slots.py`) that emits the slot lists and 238 forwarders from the staged header, the proxy, four
workloads in one binary, and `build.ps1`. Built with **`cl` + `ml64`** to `C:\Users\Rupansh\d12g5`.
⚠ The `g++` + `__declspec(naked)` recipe `DDI_REFERENCE.md` §15.2 used to give **cannot build**: GCC
has no `naked` attribute on x86-64 and the WDK headers want `cl`. §15.2 now carries the recipe that
works, including the `NTSTATUS` typedef `d3d12umddi.h` needs in user mode.

**Containment check — passes.** 23 distinct `Type` values, **all ⊆ the 43**, **none of the 7
deprecated ones**, and a per-enumerator table for all 36 live ones in `answers.md` §2 and
`DDI_REFERENCE.md` §11.1a. ⭐ The asked set is **identical in all four workloads** — caps are
answered entirely at adapter open and device creation, so "not asked" is not a workload artefact.

The results that change the plan:

| | |
|---|---|
| **negotiated version** | `_0110`, the newest of the **77** WARP offers; `Interface`/`Version` = the token's high/low 32 bits, **confirmed bit-for-bit** |
| ⭐ **version floor** | **`_0040` is ACCEPTED and a triangle presents on it** — 96 core + 58 CL slots instead of 124 + 75. §15.4's probe, run without any Helios code |
| ⭐ **shader bytecode** | the runtime hands `pfnCreateShader` a **raw stream** (`dword[0]=(type<<16)\|(major<<4)\|minor`, `dword[1]=length in dwords`, `dword[2]='DXIL'`) — **never a DXBC container**, and it converts SM 5.1 DXBC to DXIL first. **Q1 answered** |
| ⭐ **caps are one contract** | cross-cap rules ARE enforced **at retail**, at `D3D12CreateDevice`, `0x887A0020` + an English reason on ETW **`Microsoft-Windows-Direct3D12`** (not `DxgKrnl`/`AzureTriage`, which said nothing). Out-of-range tiers are **clamped silently**; legal answers reach the app verbatim. **Q2 answered** |
| **the DXGI table** | `TableType == 3` is **never requested** — 0 of 32 armed DXGI thunks called across 20 flip-model presents. A D3D12 UMD needs no DXGI table on this build |
| **7.17 / WDDM** | the runtime does **not** gate the DDI version on the adapter's WDDM level: forced `_0110` negotiated cleanly on the WDDM 2.1 **Helios** adapter, through Route B |
| **coverage** | a triangle + present touches **70 of 206** slots (47 core, 22 CL, 1 queue, 0 DXGI) |

**Known traps, all of which produced a confident wrong answer first:**
* ⛔ **Route A works** (app-local `d3d10warp.dll`, no registry, no reboot) — §15.2's UNVERIFIED is
  closed. But `Get-ChildItem "C:\ProgramData\Helios\d3d12_spy-*.log"` returns **nothing** while the
  logs exist: that directory has a junction loop. Use `-Filter`, never a wildcard inside the path.
  Two "the proxy was never loaded" readings were this glob.
* ⛔ **Route B needs `pnputil /restart-device`** after rewriting `UserModeDriverName[3]` — dxgkrnl
  uses the path it cached at StartDevice, so without the restart the proxy is simply never loaded and
  the experiment silently measures the old driver.
* ⛔ **The proxy's own gate refuses everything when `HKLM\SOFTWARE\Helios!UmdD3D12Spy` is absent.**
  Four "the runtime rejected this DDI version" results were the spy refusing itself. Assert the knob
  in the same command that runs the arm.
* ⛔ In the version-force arm, editing `pfnGetSupportedVersions`' **COUNT** answer makes the real
  driver's FILL return `ERROR_INSUFFICIENT_BUFFER` (0x8007007A), and skipping the real call entirely
  crashes it at `0xC0000005`. Edit only the FILL answer, on the way out.
* ⛔ The spy's first version capped its token capture at 64 while the driver returns **77**, and
  printed "NO MATCH" — a truncated instrument reading exactly like a finding.
* ✅ Route B was fully reverted: registry restored, knob removed, device restarted, and the desktop
  verified alive and composited with `helios_paintcap` → `Z:\tmp\screen_copy.png`.

**⚠ The workloads are `tools/d3d12_spy/spy_workload.cpp`, not `D3D12HelloWorld`.** That sample
carries the Agility SDK exports (`D3D12HelloWindow.cpp:15-16`), so which `D3D12Core.dll` it runs
against depends on whether a `D3D12\` directory sits beside the exe — and this gate is a measurement
*of* the shipping runtime. It also needs a NuGet `d3dx12.h` that is not vendored here. The
`triangle` mode additionally builds **two** pipelines in one process, SM 6.0 DXIL and SM 5.1 DXBC,
which is what settled Q1; a single shader model cannot show a conversion.
...
⛔ **A HANG is not a step on this ladder — it has its own entry point.** *(2026-08-05.)* If the guest
hangs with **no reported device removal** — the live null-UAV-counter defect — then
`ID3D12DeviceRemovedExtendedData` returns **nothing**, because DRED's API path requires a removal to
have been *detected* first. So does every dump instrument. Go straight to an ETW
`Microsoft-Windows-DxgKrnl` all-keywords slice and answer one question first: **is a DMA packet
outstanding?** If yes, TDR is being suppressed somewhere in `kmd_render`; if no, the KMD never queued
the work and dxgkrnl has nothing to time out. `GATES.md` §7.26 owns this and states the chain.
...
| ~~7.12~~ | ✅ **SETTLED 2026-08-05 by `D12-G5` — YES, and at retail.** An inconsistent caps *set* fails `D3D12CreateDevice` with `DXGI_ERROR_DRIVER_INTERNAL_ERROR` (0x887A0020) and the runtime's own English reason. Two worked failures: `FL12+ driver incorrectly did not report support for resource binding tier 2+.` and `Drivers that expose AtomicInt64OnTypedResource, … or WaveMMATier must expose shader model 6.6.` ⚠ Two things it is NOT: an out-of-range tier is **clamped silently** (tier 99 → the app sees 3, debug layer included), and a legal answer propagates **verbatim**. ⛔ The provider is **`Microsoft-Windows-Direct3D12`** — `DxgKrnl`/`AzureTriage` said nothing, the failure is above dxgkrnl. `DDI_REFERENCE.md` §11.5.0. | Closed. |
| ~~7.13~~ | ✅ **SETTLED 2026-08-05 by `D12-G5` — a RAW STREAM, always.** `dword[0] = (programType << 16) \| (major << 4) \| minor`, `dword[1] = length in dwords`, `dword[2] = 'DXIL'`; `'DXBC'` never appears. ⭐ And the runtime **converts SM 5.1 DXBC to DXIL before the DDI** — two pipelines in one process, one `dxc -T vs_6_0` and one `D3DCompile "vs_5_1"`, both arrived as `…0060` + `'DXIL'` with lengths matching neither app blob. A D3D12 UMD on this build never sees DXBC. `DDI_REFERENCE.md` §12.2. | Closed. |
| 7.14 | **Exact contract of `D3D12DDICAPS_TYPE_EXECUTECOMMANDLISTS_PARALLELISM`** (1069, `pData = BOOL`, `d3d12umddi.h:128`). Report FALSE until proven. ⚠ **Narrowed by `D12-G5`: the runtime never asks for 1069** — not in any of the four workloads, so it cannot be read off a working driver's answer and the cheap arm is gone. | The remaining arm only: force TRUE in the spy thunk and diff a `logman`/`tracerpt` `DxgKrnl` slice for overlapping `QueuePacket` submits. |
...
| 7.17 | **Does a WDDM 2.1 adapter constrain the shader models a D3D12 UMD may report?** ✅ **The bigger half is SETTLED 2026-08-05 by `D12-G5` Route B, and in the good direction: the runtime does NOT gate the DDI VERSION on the adapter's WDDM level.** WARP opened on the WDDM 2.1 Helios adapter offers only a D3D11-era token and `D3D12CreateDevice` fails — but that is **WARP's own adapter-derived policy**: forcing the answer to `_0110` on the *same* adapter made the runtime negotiate it cleanly and call `pfnCalcPrivateDeviceSize(0x000c0050, 0x006e0000) -> 4016` and a full `pfnCreateDevice`. So `Wddm2_1GpuMmu` is not by itself a barrier to `helios_umd12` reporting `_0110`. ⚠ **Still open, narrowly:** the shader-model half. WARP never reached its caps sequence on the Helios adapter, so no SM answer was ever offered there; every ceiling the runtime *did* enforce was against the OPTIONS caps, never against a WDDM version. | The narrow question moves to **G7**, on `helios_umd12.dll`'s own caps: report SM 6.8 at `Wddm2_1GpuMmu` and read whether `D3D12CreateDevice` succeeds. |
...
| 7.19 | **P-C, narrowed twice.** ✅ *Resolved:* the callback surface is **not** limited to `d3d12umddi.h` — `D3D12DDIARG_CREATEDEVICE_0109.pKTCallbacks` (`d3d12umddi.h:13623`) is a `CONST D3DDDI_DEVICECALLBACKS*`, the same 65-entry table the D3D11 UMD drives (`d3dumddi.h:4499`), and it carries **`pfnRenderCb` and `pfnPresentCb`**. So the `HeliosPresentRenderCmd` identity channel transfers with **no KMD change** (`DECISIONS.md` §6.1). ⛔ Do **not** re-open this as "decode the identity in `DxgkDdiSubmitCommandVirtual`" — that DDI is DISPATCH_LEVEL (`kmd_render/src/ddi/submit_command.rs:723-724`) where `diag::record*` is illegal. **Still open:** (a) whether the D3D12 runtime tolerates the driver calling `pfnRenderCb` around `pfnPresent`, and (b) whether `D3D12DDIARG_PRESENT_0001.pPrivateDriverData` reaches `DxgkDdiPresent` at all — the D3D11 answer is *no on DMA flips* (memory 64th), which is exactly why the identity rides the Render command. ⭐ **`D12-G5` added the missing half of (b): `core[81] pfnGetPresentPrivateDriverDataSize` is called ONCE PER PRESENT, immediately before `pfnPresent`** — that is the driver's opportunity to size a private-data buffer. WARP returns 0, so every observed present carried `PrivateDriverDataSize = 0` and `pPrivateDriverData = NULL`; a driver that returns N gets an N-byte buffer. The rest of the argument, verbatim: `surfaces=1  hDstResource=NULL  Flags=0x21  FlipInterval=0  VidPnSourceID=0xffffffff  DirtyRects=0  OptimizeForComposition=1`, and `pOut` is a **536-byte** `D3D12DDI_PRESENT_0051` whose first dword is a `D3DKMT_HANDLE`. | (a) `pfnRenderCb` + a counting `DxgkDdiRender` on the D3D12 path at **G8**, confirming the count moves — before any later rung depends on it. (b) now a **two-candidate** test at G8: return a non-zero size from `pfnGetPresentPrivateDriverDataSize`, write a watermark into the buffer, and read whether it reaches `DxgkDdiPresent`. If it does, the identity channel needs no Render command at all; if it does not, the D3D11 answer carries over and the Render command stands. ⛔ Do not drop the Render-command plan on the strength of the call existing — the call existing is not the data arriving. |
...
| 7.23 | ⭐ **Does the D3D12 runtime require a resource created by `helios_umd12.dll`/vkd3d to be openable by `helios_umd.dll`/DXVK, and vice versa?** `ResourceHeaps.md:1254-1256` states it as a driver requirement — *"D3D12-created resources must be able to be opened by DWM, using D3D11 and the 11 DDI"* — and `:198` says *"The driver must construct private data that is consumable by their D3D11 driver"*. `DECISIONS.md` **D3c** records the constraint. Helios has two DLLs and two independent engines, and DWM composites the whole desktop, so this is on the **P4** critical path. ⚠ What is NOT established: whether the runtime enforces it at create time or only when a handle is actually opened. | Create a shared texture through `helios_umd.dll`/DXVK (`D3D11 CreateSharedHandle`), then open it through `helios_umd12.dll`/vkd3d via `pfnOpenHeapAndResource`; then the reverse direction. Count refusals on both arms. ⛔ **Not** an 11on12 test — `TranslationLayerResourceInterop.md` covers only Microsoft's own layers, where there is exactly one driver-side engine. **G7, before G8 depends on a D3D12 back buffer reaching DWM.** |
...
| 7.29 | ⭐ **Should `helios_umd12.dll` statically link vkd3d instead of `LoadLibrary`-ing `helios_vkd3d.dll`?** `DECISIONS.md` D4's rationale was reopened 2026-08-05: reason 2 is retracted, reason 3 is subsumed by D3, and reason 1 is **backwards** — static archive linking would drop the `CreateDXGIFactory1` import that the DLL build currently has, because the only object referencing it is the DLL's own `main.c.obj` (measured: 0 references across all five engine archives). vkd3d's meson already supports **clang-cl** (`meson.build:9`), the compiler `dxvk-helios` is built with, and both engines are our forks — `dxvk-helios` already carries a Helios-only `libhelios_d3d11_static.a` target that upstream lacks. What remains is cost, not principle. | Build `helios_d3d12_static` per `research/R4.md` §6.4 with clang-cl + `-Db_vscrt=md`, matching DXVK. Report three numbers: (a) does it build at all across the seven repos — and what did `widl`/`glslang` cost on Windows (`GATES.md` §7.9); (b) `objdump -p` on the resulting `helios_umd12.dll` — the pass condition is **no `dxgi.dll` import**; (c) the DLL size, against the 105 KB the LoadLibrary shape gives today, because dwm calls `OpenAdapter12` in production (`DECISIONS.md` §7.13) and would map the whole thing. ⚠ Keep the mingw cross-build as the conformance arm regardless — upstream says it does not stress-test MSVC builds. **Before S4 writes the bridge**, since the bridge's shape differs between the two arms. |
| 7.28 | ⛔ **`helios_vkd3d.dll` statically imports `CreateDXGIFactory1` from `dxgi.dll`, so loading the engine loads DXGI — which is the exact thing `DECISIONS.md` D4 reason 1 exists to prevent.** Measured 2026-08-05 with `objdump -p` on the G0 build: the symbol sits in *The Import Tables*, and the delay-import directory is empty, so it binds at **load** time regardless of whether `helios_vkd3d_create_device` calls it. Bypassing the call was never sufficient — `libs/d3d12core/main.c` is still linked into the target, and an import is a property of the link, not the call graph. ⚠ Whether it is actually *harmful* is a separate question: the hazard D4 names is a UMD that loads dxgi during device creation **re-entering adapter enumeration that loads the UMD**, and dxgi being mapped is not the same as dxgi being driven. | Two arms, and do the cheap one first. **(a)** Add a `helios_vkd3d` meson target that excludes `libs/d3d12core/main.c` — the same object-exclusion `research/R4.md` §6.4 already specifies for the static fallback — rebuild, and confirm `objdump -p \| grep dxgi` is empty. If the two Helios exports still resolve and `D12-G1`'s bridge probe still passes, the question is closed by construction and no measurement of the hazard is needed. **(b)** Only if (a) is impossible: instrument `pfnCreateDevice` on the D3D12 path and prove no re-entrant `OpenAdapter`/`OpenAdapter12` occurs while dxgi is resident. ⛔ (b) is a claim about the loader and the D3D12 runtime together and needs a measurement, not an argument. **Before G7 depends on the bridge.** |
