...
└── src/
    ├── lib.rs
    ├── ddi12.rs
    ├── adapter12.rs                 # ⛔ D13: private data that CROSSES a module boundary is
    │                                #   helios_protocol's (HeliosWddmAllocPrivate / OpenIdentity /
    │                                #   PresentRenderCmd) and is reused verbatim — never
    │                                #   re-declared here. Per-object pDrvPrivate blocks stay local.
...
| `src/device12.rs` | `HeliosD3D12Device` (the private block), `device_private_size()`, the `pfnFillDDITable` dispatch, per-queue `RuntimeContext` creation, `DestroyDevice`. ⚠ **D13: this block stays here** — it is per-object, per-process and crosses no boundary. What must NOT be re-declared here is the *crossing* private data (`HeliosWddmAllocPrivate`, `HeliosWddmOpenIdentity`, `HeliosPresentRenderCmd`), which `helios_protocol` already owns | `umd/src/device_funcs.rs` (1318 L) | same runtime-allocated-private-block model (R2 §1.1); context creation is per **queue** not per device (R2 §1.3) |
