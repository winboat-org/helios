...
try {
    ...
    Write-TestFile (Join-Path $resolve "atiadlxx.dll") "third-party modification"
    $blocked = $false
    try {
    ...
    Assert-True ((Get-Content -LiteralPath (Join-Path $resolve "atiadlxx.dll") -Raw) -match "third-party") `
        "blocked uninstall must not overwrite the modified DLL"

    Copy-Item -LiteralPath (Join-Path $package "atiadlxx.dll") -Destination (Join-Path $resolve "atiadlxx.dll") -Force
    & (Join-Path $state "Uninstall-Resolve-Compatibility.ps1") -StateDirectory $state
